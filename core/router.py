...
# 在模块级别导入 MemoryManager，便于测试通过 patch('core.router.MemoryManager') 进行替换。
# 如果 memory 包尚不可用，则延迟为 None，并在使用时再尝试导入。
try:
    from memory import MemoryManager
except Exception:  # pragma: no cover - 导入失败时降级
    MemoryManager = None
...
# 向后兼容：允许 `from core.router import EnhancedRouter`
from core.enhanced_router import EnhancedRouter  # noqa: E402,F401
