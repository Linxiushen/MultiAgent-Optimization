...
class HistoryBasedStrategy(RoutingStrategy):
    ...
    def get_query_similarity(self, query: str) -> Dict[str, float]:
        ...
        Returns:
            各Agent的查询相似度得分
        """
        # 基于字符级 Jaccard 相似度计算，兼容中文（中文查询通常没有空格，
        # 无法按空白分词），同时对英文也适用。
        ...
        for interaction in self.interaction_history:
            ...
            hist_tokens = self._tokenize(hist_query)

            # 计算Jaccard相似度
            if hist_tokens and query_tokens:
                intersection = len(query_tokens.intersection(hist_tokens))
                union = len(query_tokens.union(hist_tokens))
...
class SemanticRoutingStrategy(RoutingStrategy):
    ...
    def select_agent(self, query: str, agent_profiles: Dict[str, Dict[str, Any]], 
        ...
        # "general" 意图作为兜底意图：只有在没有任何专门意图匹配时才使用，
        # 否则匹配到专门意图（如 technical / creative）的 Agent 优先。
        fallback_intent = "general"
        specialized_matched = any(
            score > 0.0
        ...
        for agent_id, profile in agent_profiles.items():
            ...
            # 计算意图匹配得分：取该Agent支持的意图中的最高得分
            # （避免支持意图较多的 Agent 因为求平均而被稀释）。
            intent_match_score = 0.0
            for intent in supported_intents:
                # 若已有专门意图匹配，则忽略兜底意图的贡献
                if specialized_matched and intent == fallback_intent:
                    continue
                intent_match_score = max(intent_match_score, intent_scores.get(intent, 0.0))
        ...
        # 当所有相似度相同（如使用占位 embedding）时，意图匹配决定结果。
        return max(scores.items(), key=lambda x: x[1])[0]
...
class MultiAgentRoutingStrategy(RoutingStrategy):
    ...
    def decompose_task(self, query: str) -> List[Dict[str, Any]]:
        ...
        # 按句子切分（兼容中英文标点）。
        import re
        sentences = [s for s in re.split(r'[。！？!?]|\.\s', query) if s.strip()]

        # 检测查询是否需要分解：词数较少且只有一个句子时视为简单查询。
        if len(sentences) <= 1 and len(query.split()) < 10:
            return [{
        ...
        for i, sentence in enumerate(sentences):
            ...
            sub_tasks.append({
                "sub_query": sentence.strip(),
                "type": "decomposed",
                "order": i
            })

        # 兜底：若切分后没有得到任何子任务，则视为单一简单任务。
        if not sub_tasks:
            return [{
                "sub_query": query.strip(),
                "type": "simple"
