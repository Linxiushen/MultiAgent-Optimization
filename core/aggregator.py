...
class Aggregator:
    ...
    def aggregate_responses(self, agent_responses: List[Dict[str, Any]], query: str = "") -> str:
        """聚合多个Agent的响应并返回最终文本响应。

        基础实现复用 ``fuse_responses`` 的融合结果，提取其中的文本内容。

        Args:
            agent_responses: Agent响应列表
            query: 原始查询（保留参数以兼容增强版接口）

        Returns:
            str: 聚合后的文本响应
        """
        fused = self.fuse_responses(agent_responses)
        if isinstance(fused, dict):
            return fused.get("response") or fused.get("content") or json.dumps(fused, ensure_ascii=False)
        return str(fused)
    ...
    def _confidence_based_selection(self, agent_responses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "source": best_response.get("agent_id") or best_response.get("source")
        }
    ...
    def generate_final_response(self, fused_response: Dict[str, Any], query: str) -> str:
        ...
        # 添加融合的响应内容（content）
        context_parts.append("响应内容(content):")
        context_parts.append(fused_response.get("content", ""))

        # 添加置信度信息（confidence）
        context_parts.append(f"置信度(confidence): {fused_response.get('confidence', 0.5):.2f}")

        # 添加来源信息（sources）
        sources = fused_response.get("sources")
        if sources is None and fused_response.get("source") is not None:
            sources = [fused_response.get("source")]
        if sources:
            source_labels = [str(s) for s in sources if s is not None]
            context_parts.append(f"来源(sources): {', '.join(source_labels)}")
        else:
            context_parts.append("来源(sources): ")
...
# 向后兼容：允许 `from core.aggregator import EnhancedAggregator`
from core.enhanced_aggregator import EnhancedAggregator  # noqa: E402,F401
