...
class FactVerifier:
    ...
    def _verify_with_external_api(self, statement: str) -> Tuple[bool, float, Optional[str]]:
        ...
        try:
            ...
            # 仅当 status_code 是明确的非 200 整数时才视为失败，
            # 否则解析响应内容（兼容未显式设置 status_code 的响应）
                            ...
                            for j in range(i+1, len(objects)):
                                ...
                                for fact in obj1_facts:
                                    if fact["object"] == objects[j] or fact["subject"] == objects[j]:
                                        ...
                                        return {
                                            "verified": True,
                                            "resolution": f"两个陈述实际上是一致的，表达方式不同: {statements[0]} 和 {statements[1]}",
                                            "confidence": 0.7,
    ...
    def _relation_to_text(self, relation: str) -> str:
        """将关系类型转换为中文连接词"""
        mapping = {
            "is_a": "是",
            "has_property": "包含",
        ...
        return mapping.get(relation, "相关于")

    def get_related_facts(self, entity: str, max_distance: int = 2) -> Dict[str, Any]:
        """获取与实体相关的事实（包括直接和间接相关）
        ...
        for _ in range(max_distance):
            ...
            for node in frontier:
                for related in self.knowledge_graph.get_related_facts(node):
                    key = (related["subject"], related["relation"], related["object"])
                    if key in seen:
                        continue
                    seen.add(key)
                    statement = (
                        f"{related['subject']}"
                        f"{self._relation_to_text(related['relation'])}"
                        f"{related['object']}"
                    )
                    facts.append({
                    ...
                    # 沿主体方向继续扩展，以获取间接相关事实
                    if related["subject"] == node:
                        next_frontier.append(related["object"])
                    else:
                        next_frontier.append(related["subject"])
