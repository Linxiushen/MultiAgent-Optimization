...
# 以裸名导入，便于测试通过 patch('core.enhanced_router.call_model') 等方式替换；
# 同时不影响对 adapters.llm 内部逻辑的复用。
...
class EnhancedRouter:
    ...
    def _llm_based_routing(self, query: str, context: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        # 使用轻量级LLM推理（LLM不可用时返回 None，做兜底避免崩溃）
        response = (lite_llm_infer(prompt) or "").strip()
    ...
    def route_query(self, query: str, context: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
        ...
        # 当默认策略本身是一个已注册的高级策略（如 semantic/history/
        # adaptive/multi_agent）时，它优先于按Agent配置的 mapping；
        # 否则使用 agent_strategy_mapping，并回退到默认策略名称。
        if self.default_strategy_name in self.strategy_instances:
            strategy_name = self.default_strategy_name
        else:
    ...
    def execute_route(self, query: str, context: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
        ...
        # 通过协调器进行可用性检查、备选选择与任务分配
        agent_id = self._resolve_agent_with_coordinator(agent_id)
    ...
    def _resolve_agent_with_coordinator(self, agent_id: str) -> str:
        """借助协调器解析最终使用的Agent。

        - 如果未注入协调器，直接返回原Agent。
        - 如果首选Agent不可用，则在其它候选Agent中选择一个可用的。
        - 如果没有任何可用Agent，则回退到 fallback_agent。
        - 选定后通过 assign_task 进行任务分配。

        Args:
            agent_id: 路由初步选定的Agent ID
        ...
        # 优先尝试首选Agent
        try:
            if coordinator.is_agent_available(agent_id):
                selected = agent_id
        except Exception:
            selected = None

        # 首选不可用时，在候选Agent中寻找可用者
        if selected is None:
            candidates = []
            try:
                candidates = list(coordinator.get_available_agents() or [])
            except Exception:
                candidates = []

            # 退化到已配置的Agent列表作为候选来源
            if not candidates:
                candidates = list(self.agent_profiles.keys())

            for candidate in candidates:
                if candidate == agent_id:
                    continue
                try:
                    if coordinator.is_agent_available(candidate):
                        selected = candidate
                        break
                except Exception:
                    continue
        ...
        # 所有Agent都不可用时回退
        if selected is None:
            selected = self.fallback_agent

        # 分配任务
        try:
            coordinator.assign_task(selected)
        except Exception:
            pass
    ...
    def _execute_direct_strategy(self, query: str, agent_id: str, model: str, temperature: float) -> Dict[str, Any]:
        ...
        response = call_model(prompt, model_name=model, temperature=temperature)
    ...
    def _execute_multi_agent_strategy(self, query: str, context: Optional[List[Dict[str, str]]], 
        ...
        # 为子任务分配Agent：
        # - 若子任务自身已带有 agent_id，则直接使用；
        # - 否则使用策略进行分配。
        if sub_tasks and all(isinstance(t, dict) and t.get("agent_id") for t in sub_tasks):
            task_agent_pairs = [(t, t["agent_id"]) for t in sub_tasks]
        else:
        ...
        for task, agent_id in task_agent_pairs:
            ...
            # 调用模型
            response = call_model(prompt, model_name=agent_model, temperature=agent_temp)
        ...
        # 聚合结果：优先使用注入的聚合器，否则使用策略自带的聚合逻辑。
        agents_used = list({pair[1] for pair in task_agent_pairs})
        if self.aggregator is not None and hasattr(self.aggregator, "aggregate_responses"):
            aggregated_response = self.aggregator.aggregate_responses(
        ...
        # 标注所使用的策略，便于上层判断。
        aggregated_result["strategy"] = "multi_agent"
