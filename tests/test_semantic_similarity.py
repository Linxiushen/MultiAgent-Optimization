...
def test_semantic_similarity():
    ...
    for query in queries:
        ...
        for i, (mid, score) in enumerate(relevant_memories):
            print(f"  {i+1}. {memories[mid]['content'][:50]}...")
...
def test_memory_optimization_pipeline():
    ...
    # 3. 最后进行遗忘操作
    print("\n3. 执行低优先级记忆遗忘...")
    before_count = len(compressed_memories)
    forgotten_ids = optimizer.forget_low_priority_memories(compressed_memories, keep_ratio=0.6)
    kept_memories = compressed_memories  # forget 会原地删除低优先级记忆
    print(f"遗忘前记忆数量: {before_count}")
    ...
    for query in queries:
        ...
        print(f"找到 {len(relevant_memories)} 条相关记忆:")
        for i, (mid, score) in enumerate(relevant_memories):
            print(f"  {i+1}. {kept_memories[mid]['content'][:50]}...")
