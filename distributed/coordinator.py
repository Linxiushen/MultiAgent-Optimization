...
class DistributedCoordinator:
    ...
    def init_failover_strategy(self) -> None:
        ...
        if strategy_type == 'failover':
            ...
            self.failover_strategy = BackupAgentStrategy()
        else:
    ...
    def get_available_agents(self) -> List[str]:
        """返回当前处于活跃状态的Agent ID列表。

        Returns:
            List[str]: 活跃Agent的ID列表
        """
        return [
            agent_id
    ...
    def is_agent_available(self, agent_id: str) -> bool:
        """判断指定Agent当前是否可用（已注册且处于活跃状态）。

        Args:
            agent_id: 待检查的Agent ID
    ...
    def assign_task(self, task_id: Optional[str] = None, task_type: str = None, task_data: Dict = None, priority: int = 1, callback: Optional[Callable] = None) -> Optional[str]:
        """分配任务给合适的Agent

        Args:
            task_id: 任务唯一标识符（可选，未提供时自动生成）
            task_type: 任务类型
            task_data: 任务数据
        ...
        # 根据负载均衡策略选择Agent
        selected_agent = self._select_agent(suitable_agents, {'type': task_type, 'data': task_data, 'priority': priority})

        # 创建任务（未提供task_id时自动生成）
        if task_id is None:
        ...
        # 将任务加入队列（兼容 PriorityQueue 与 list 两种容器）
        self._enqueue_task((priority, time.time(), task))
    ...
    def _enqueue_task(self, entry: Tuple) -> None:
        """将任务条目放入任务容器（兼容 PriorityQueue 与 list）

        Args:
            entry: (priority, timestamp, task) 三元组
        """
    ...
    def _select_agent(self, agent_ids: List[str], task: Optional[Dict] = None) -> str:
        """根据负载均衡策略选择Agent

        Args:
            agent_ids: 候选Agent ID列表
            task: 任务信息（可选），供需要任务上下文的策略使用
        ...
        if task is None:
            task = {}
        selected = self.load_balancing_strategy.select_agent(agent_info, task)
        # 防御性校验：策略返回的Agent必须在候选列表内，否则回退到首个候选
        if selected not in agent_info:
    ...
    def acquire_lock(self, resource_id: str, agent_id: str, timeout: float = 10.0) -> bool:
        ...
        Returns:
            bool: 是否成功获取锁
        """
        # 参数顺序容错：若第一个参数实为已注册的Agent而第二个不是，
        # 说明调用方传入的是 (agent_id, resource_id) 顺序，自动纠正。
        if resource_id in self.agents and agent_id not in self.agents:
            resource_id, agent_id = agent_id, resource_id
    ...
    def _monitor_agents(self):
        """执行一次Agent状态检查，标记超时Agent并触发故障转移"""
        current_time = time.time()
        heartbeat_interval = self.config.get('agent_heartbeat_interval', 30)
    ...
    def _handle_timeout_agent(self, agent_id: str):
        ...
        # 根据当前配置确保故障转移策略与配置一致，避免跨调用/跨测试的状态泄漏
        if use_failover_strategy:
            self.init_failover_strategy()

        # 查找所有未完成的任务（兼容 PriorityQueue 与 list 两种容器）
        pending_tasks = []
        remaining_tasks = []

        # 取出当前队列中的所有任务条目
        if isinstance(self.tasks, list):
            all_entries = list(self.tasks)
            self.tasks.clear()
        else:
            all_entries = []
            while not self.tasks.empty():
                all_entries.append(self.tasks.get())

        # 遍历任务条目，找出分配给超时Agent的任务
        for priority, timestamp, task in all_entries:
        ...
        # 恢复其他任务到队列
        for entry in remaining_tasks:
            self._enqueue_task(entry)
        ...
        for priority, timestamp, task in pending_tasks:
            ...
            if new_agent:
                ...
                # 注意：reassignment_count 与 previous_agents 已在上方更新，
                # 此处不可重复递增，否则计数会翻倍。
                ...
                if task['reassignment_count'] <= max_retries:
                    ...
                    # 将任务重新加入队列
                    self._enqueue_task((priority, timestamp, task))
                    ...
                    if use_failover_strategy and self.failover_strategy and \
                            hasattr(self.failover_strategy, 'update_failover_history'):
                ...
                # 将任务放回队列，但降低优先级
                self._enqueue_task((priority + 5, timestamp, task))
    ...
    def _select_agent_for_failover(self, agent_ids: List[str], task: Dict) -> str:
        ...
        return self._select_agent(available_agents, task)
    ...
    def _determine_failure_severity(self, failure_event: Dict) -> str:
        ...
        # 将当前正在评估的故障事件计入（若其尚未记录到历史中）
        if failure_event not in getattr(self, 'failure_history', []):
            recent_failures += 1
    ...
    def get_cluster_status(self) -> Dict:
        ...
        return {
            ...
            'pending_tasks': len(self.tasks) if isinstance(self.tasks, list) else self.tasks.qsize(),
...
# 向后兼容别名：部分模块/测试以 Coordinator 引用协调器
