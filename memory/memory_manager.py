...
class MemoryManager:
    def __init__(self, memory_dir: str = "memory/store", index_path: str = "memory/index.faiss"):
        ...
        # 与FAISS索引并行维护的记忆ID列表，第i个元素对应索引中的第i个向量
        self.ids_path = index_path + ".ids.json"
    ...
    def _save_index(self) -> None:
        """保存FAISS索引以及并行的记忆ID列表"""
        faiss.write_index(self.index, self.index_path)
        with open(self.ids_path, 'w', encoding='utf-8') as f:
            json.dump(self.memory_ids, f, ensure_ascii=False)
    ...
    def _generate_embedding(self, text: str) -> np.ndarray:
        ...
        # LLM不可用时返回 None，json.loads(None) 会抛 TypeError；与 JSON 解析错误一并兜底
        try:
    ...
    def store_memory(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        Returns:
            记忆ID
        """
        # 使用毫秒时间戳加上唯一后缀，避免同一毫秒内连续写入产生ID冲突
    ...
    def retrieve_memory(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        # 检索记忆内容
        collected = []  # (插入顺序索引, memory_data)
        memory_dict = {}
        for i, idx in enumerate(indices[0]):
            # FAISS返回的是索引位置，需要映射回真实的记忆ID
            ...
            if os.path.exists(memory_file):
                with open(memory_file, 'r', encoding='utf-8') as f:
                    ...
                    collected.append((int(idx), memory_data))
                    memory_dict[memory_data['id']] = memory_data

        # 当多个记忆距离几乎相同时（FAISS 浮点精度会引入极微小差异），
        # 以稳定的插入顺序作为次级排序依据，保证检索结果可复现。
        collected.sort(key=lambda x: x[0])
        memories = [m for _, m in collected]
    ...
    def optimize_memories(self) -> Tuple[List[Dict[str, Any]], List[str]]:
        ...
        self.index = faiss.IndexFlatL2(self.dimension)
        self.memory_ids = []
        for mem_id, memory in optimized_memories.items():
            # 重新生成嵌入并添加到索引
